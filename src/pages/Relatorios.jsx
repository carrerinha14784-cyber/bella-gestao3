...
function gerarAnos(vendas) {
  ...
  if (anos.size === 0) {
    ...
    return [String(ano), String(ano - 1)];
  }
...
export default function Relatorios() {
  ...
  const resumoMensal = MESES.map((nomeMes, idx) => {
    ...
    return { nomeMes, total, qtd: vMes.length };
  });
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .subtitulo { color: #888; font-size: 12px; margin-bottom: 24px; }
          ...
          th { background: #f4f6f8; padding: 8px 10px; text-align: left; border-bottom: 2px solid #ddd; font-weight: 600; color: #444; }
  ...
  const periodoLabel =
    modo === "mensal" ? `${MESES[mesSel]} / ${anoSel}` : `Ano ${anoSel}`;
  ...
  return (
    <div style={estilos.pagina}>
      <div style={estilos.cabecalhoTela}>
        ...
        <button style={estilos.btnExportar} onClick={exportarPDF}>
          ⬇️ Exportar PDF
        </button>
      </div>
      ...
      <div style={estilos.controles}>
        <div style={estilos.modoOpcoes}>
          {["mensal", "anual"].map((m) => (
            <button
              key={m}
              style={modo === m ? estilos.btnModoAtivo : estilos.btnModo}
              onClick={() => setModo(m)}
            >
      ...
      <div ref={printRef}>
        ...
        <div style={{ marginBottom: "20px" }}>
          <h2 style={estilos.relatorioTitulo}>
            Bella Gestão — Relatório {modo === "mensal" ? "Mensal" : "Anual"}
          </h2>
        ...
        <div style={estilos.avisoFiscal}>
          ⚠️ <strong>Documento de uso interno.</strong> Este relatório não possui valor fiscal ou legal.
          Consulte um contador para declaração de imposto de renda.
        </div>
        ...
        <div style={estilos.secao}>
          ...
          {Object.keys(porPagamento).length === 0 ? (
            ...
            <table style={estilos.tabela}>
              <thead>
                <tr>
                  {["Forma de Pagamento", "Qtd Vendas", "Total (R$)", "% do Total"].map((h) => (
        ...
        <div style={estilos.secao}>
          ...
          <table style={estilos.tabela}>
            <thead>
              <tr>
                {["Situação", "Valor (R$)"].map((h) => <th key={h} style={estilos.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              <tr style={estilos.tr}>
        ...
        {/* Resumo mensal (apenas no modo anual) */}
        {modo === "anual" && (
          <div style={estilos.secao}>
            ...
            <table style={estilos.tabela}>
              <thead>
                <tr>
                  {["Mês", "Nº Vendas", "Total Bruto (R$)"].map((h) => (
                    <th key={h} style={estilos.th}>{h}</th>
                  ))}
                </tr>
              </thead>
        ...
        <div style={estilos.secao}>
          ...
          {vendasPeriodo.length === 0 ? (
            ...
            <table style={estilos.tabela}>
              <thead>
                <tr>
                  {["Data", "Cliente", "Pagamento", "Status", "Total (R$)"].map((h) => (
                    <th key={h} style={estilos.th}>{h}</th>
                  ))}
                </tr>
              </thead>
...
function CardResumo({ icone, label, valor, cor }) {
  return (
    <div style={{ ...estilos.card, borderLeft: `4px solid ${cor}` }}>
      <span style={estilos.cardIcone}>{icone}</span>
      <div>
        <p style={estilos.cardLabel}>{label}</p>
        <p style={{ ...estilos.cardValor, color: cor }}>{valor}</p>
      </div>
    </div>
  );
}
...
const estilos = {
  ...
  cardIcone: { fontSize: "26px" },
  cardLabel: { fontSize: "12px", color: "#888", margin: "0 0 4px 0" },
  cardValor: { fontSize: "20px", fontWeight: "700", margin: 0 },
  secao: {
    background: "#fff",
    borderRadius: "12px",
    padding: "20px",
