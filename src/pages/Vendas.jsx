...
const STORAGE_PRODUTOS = "bella_produtos";
const STORAGE_CLIENTES = "bella_clientes";
...
export default function Vendas() {
  ...
  useEffect(() => {
    const p = localStorage.getItem(STORAGE_PRODUTOS);
    const c = localStorage.getItem(STORAGE_CLIENTES);
    if (p) setProdutos(JSON.parse(p));
    if (c) setClientes(JSON.parse(c));
  }, []);

  // Produtos filtrados pela busca, excluindo sem estoque
  ...
  function finalizarVenda() {
    ...
    const produtosAtualizados = produtos.map((p) => {
      ...
      if (itemCarrinho) {
        return { ...p, estoque: Number(p.estoque) - itemCarrinho.quantidade };
      }
      return p;
    });
    localStorage.setItem(STORAGE_PRODUTOS, JSON.stringify(produtosAtualizados));
    ...
    const vendasSalvas = localStorage.getItem(STORAGE_VENDAS);
    const vendas = vendasSalvas ? JSON.parse(vendasSalvas) : [];
    localStorage.setItem(STORAGE_VENDAS, JSON.stringify([novaVenda, ...vendas]));
              ...
              <div style={estilos.listaBusca}>
                {produtosFiltrados.length === 0 ? (
                  ...
                  produtosFiltrados.map((p) => (
                    <div
                      key={p.id}
                      style={estilos.itemBusca}
                      onClick={() => adicionarAoCarrinho(p)}
                    >
                      <span style={estilos.itemBuscaNome}>{p.nome}</span>
                      <span style={estilos.itemBuscaInfo}>
                        {p.tamanho} · {p.cor} · Estoque: {p.estoque}
                      </span>
                      <span style={estilos.itemBuscaValor}>
                            ...
                            <td style={estilos.td}>
                              <div style={estilos.qtdControle}>
                                ...
                                <button
                                  style={estilos.btnQtd}
                                  onClick={() => alterarQuantidade(item.id, item.quantidade + 1)}
                                  disabled={item.quantidade >= estoqueMax}
                                >+</button>
                              </div>
                            </td>
                            <td style={estilos.td}>
